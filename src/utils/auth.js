...
export const ADMIN_APP_URL = trimTrailingSlash(
  import.meta.env.VITE_ADMIN_APP_URL || currentOrigin,
);

export const USER_APP_URL = trimTrailingSlash(
  import.meta.env.VITE_USER_APP_URL ||
    replaceSubdomain(ADMIN_APP_URL, ADMIN_SUBDOMAIN, USER_SUBDOMAIN),
);
...
export const buildAdminGoogleLoginUrl = () => {
  ...
  if (ADMIN_APP_URL) {
    loginUrl.searchParams.set(
      "redirect_url",
      `${ADMIN_APP_URL}${GOOGLE_CALLBACK_PATH}`,
    );
  }
...
const getAuthUserFromResponse = (payload) =>
  payload?.data?.user || payload?.user || null;

const getAuthTokenFromResponse = (payload) =>
